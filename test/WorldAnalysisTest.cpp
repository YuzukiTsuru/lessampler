...
#include "../lib/World/tools/parameterio.h"
...
int main(int argc, char *argv[]) {
    ...
    int x_length = GetAudioLength("../../test/vaiueo2d.wav");
    double *x = new double[x_length];

    // wavread() must be called after GetAudioLength().
    int fs, nbit;
    wavread("../../test/vaiueo2d.wav", &fs, &nbit, x);
    ...
    WriteSpectralEnvelope("a", fs, world_parameters.f0_length, world_parameters.frame_period, 1024, 0, world_parameters.spectrogram);

    auto out = new double *[world_parameters.f0_length];
    for (int i = 0; i < world_parameters.f0_length; ++i) {
        out[i] = new double[world_parameters.fft_size / 2 + 1];
    }

    ReadSpectralEnvelope("a", out);

    for (int i = 0; i < world_parameters.f0_length; ++i) {
        for (int j = 0; j < world_parameters.fft_size / 2 + 1; ++j) {
            std::cout << i << ": " << world_parameters.spectrogram[i][j] << " -> " << out[i][j] << std::endl;
        }
    }
